...
本设计已于 2026-08-22 确认。第一阶段范围已同步到需求文档、系统设计、详细设计和实施计划；T17 已完成实现、自动化检查、可执行真机验收和用户人工兼容性补验，验收证据见 `08-selection-translation-acceptance.md`。
...
- [x] `轻译：打开面板` 已由系统服务列表识别。
- [x] 轻译未运行时，触发服务可启动应用并显示面板。
- [x] 正常选区原样进入输入框并自动执行直译和转写。
- [x] 两路结果流式显示，停止和错误行为与现有面板一致。
- [x] 仅空白选区不发起请求。
- [x] 超过已确认上限的选区只填入面板，不自动请求。
- [x] 翻译中再次触发时，旧任务保存为 `stopped`，新任务按规则开始。
- [x] 隐藏面板不停止任务，再次显示时状态和结果仍在。
- [x] 第一阶段全过程不修改通用剪贴板。
- [x] 历史开关开启和关闭时的行为均正确。

### 14.2 系统与权限

- [x] 不授予「辅助功能」权限时，第一阶段仍可在兼容应用中工作。
- [x] 应用保持 `LSUIElement` 行为，不出现在 Dock 和 `Command+Tab` 中。
- [x] 服务触发不改变开机自启、设置窗口、历史窗口和全局快捷键行为。
- [x] Release 应用安装更新后，服务列表可刷新并使用。

### 14.3 回归与证据

- [x] 单元测试覆盖外部文字载入、长文本分支和任务替换顺序。
- [x] 真机验证兼容性清单，并保存逐应用结果。
- [x] 验证 Debug、Release 构建和严格签名。
- [x] 运行现有面板、设置、历史和菜单栏视觉回归，确认基准未变化。
- [x] 检查日志不包含原文、结果或 API Key。
