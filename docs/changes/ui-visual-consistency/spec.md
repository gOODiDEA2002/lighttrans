...
- [ ] 文档、设计说明和代码使用同一菜单栏符号与布局规则。
- [ ] 面板在输入高度 70、100、240 pt 时均无裁切，两个结果区等高且可滚动。
- [ ] 面板 idle、streaming、done、partial-fail、stopped 状态层级清晰。
- [ ] 测试连接可在界面中取消，成功和长错误文案不横向挤压。
- [ ] 历史搜索位于左侧列表，支持清空、计数、刷新和正确空状态。
- [ ] 三个窗口的卡片、复制和状态视觉语言一致。
- [ ] 拖拽手柄和状态点具备必要的 VoiceOver 语义。
- [ ] `git diff --check`、`swift build`、Release 打包和严格签名校验通过。
- [ ] 不存在安装、提交、推送、依赖或数据格式变更。
