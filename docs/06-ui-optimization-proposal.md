...
- **组件垂直排版与高度**：
  1. 输入卡片：固定高度 `100 pt`（含内部 TextEditor、一键清空按钮与「82 字符」统计）。
  2. 间距：`8 pt`。
  3. 操作栏：固定高度 `28 pt`（左侧 ProgressView +「正在生成…」；右侧「停止」带 `stop.fill` 图标与红底 tint，空闲时为「翻译 ⌘↩」）。
  4. 间距：`8 pt`。
  5. 直译结果卡片：平分剩余空间，高度约 `190 pt`（头部 22 pt + 独立滚动文字区，头部带 `character.book.closed` 与「正在直译…」/「已完成」/「失败：文案」）。
  6. 间距：`8 pt`。
  7. 转写结果卡片：平分剩余空间，高度约 `190 pt`（头部 22 pt + 独立滚动文字区，头部带 `sparkles` 与「正在转写…」/「已完成」/「失败：文案」）。
  8. 间距：`8 pt`。
  9. 底部辅助栏：固定高度 `20 pt`（左侧「⌘↩ 翻译 · Esc 隐藏」；右侧 `gearshape` 与 `clock` 直达按钮）。
- **校验总高度**：`100 + 8 + 28 + 8 + 190 + 8 + 190 + 8 + 20 + 28 (上下padding) = 588 pt`，预留 `12 pt` 动态余量由双结果卡平分，完美契合 `600 pt` 窗口。
...
- **接口配置**：
  ...
  - 连通性测试：点击「测试连接」使用当前正在编辑的值发起 `max_tokens: 5` 探测请求（超时 10s，支持取消，**绝对不写历史记录**）。界面展示「当前配置连接成功 (xxx ms)」或具体错误。提示文本注明「测试将发送极简请求，可能产生极微量 API 费用」。
- **提示词模板**：
...
func testConnection(baseURL: String, model: String, apiKey: String) async throws -> TimeInterval {
    // 构造 max_tokens: 5 请求，不写入 HistoryStore，超时 10 秒
}
```

---

## 4. 实施任务分解（UI-1 至 UI-3）
